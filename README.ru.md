...
**Его честный предел:** если сокет числится за **мёртвым** PID, значит дескриптор унаследовала живая сессия Claude Code. Сторож это записывает в лог и останавливается, а не убивает твой редактор — такой сокет освободится сам, когда эти сессии закроются.
...
| Порт слушает PID, которого не существует (`taskkill: process not found`) | Осиротевший сокет — дескриптор унаследовал дочерний процесс и пережил владельца | Убить процесс-наследник; если это сессия Claude Code — сокет освободится при её закрытии |
